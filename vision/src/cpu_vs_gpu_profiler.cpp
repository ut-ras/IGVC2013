...
public:
    ...
    Proflier(bool w) : it_(nh_)
    {
        image_pub_ = it_.advertise("vision/canny", 1);
        image_sub_ = it_.subscribe("usb_cam/image_raw", 1, &Proflier::profile, this);
        pub_g_ = nh_.advertise<std_msgs::UInt32>("gpu_profile",1);
        pub_c_ = nh_.advertise<std_msgs::UInt32>("cpu_profile",1);
        SHOW_IMAGES = w;
        if (SHOW_IMAGES) {
    ...
    void profile(const sensor_msgs::ImageConstPtr& msg)
    {
        Mat src, dest;
        std_msgs::UInt32 gpu,cpu;      
        ...
        etime = ros::Time::now();
        gpu.data = ((etime.sec-stime.sec) * 1000000000) + (etime.nsec - stime.nsec);
        
        stime = ros::Time::now();
        dest = processImageCPU(src);
        etime = ros::Time::now();
        cpu.data = ((etime.sec-stime.sec) * 1000000000) + (etime.nsec - stime.nsec);
    ...
    Mat processImageCPU(Mat src)
    {
        Mat gray, blur, cannyEdges, dilated;
        GaussianBlur(src, blur, Size(11,11), 5, 5);
        cvtColor(blur, gray, CV_BGR2GRAY);
        Canny( gray, cannyEdges, 17, 17*3, 3 );
        dilate(cannyEdges, dilated, Mat::ones(3, 3, CV_8U));
        return dilated;
    }
};
