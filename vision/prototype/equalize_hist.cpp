    ...
    public:
        ...
        void laneDetectorCallback (const sensor_msgs::ImageConstPtr& msg) {
            ...
            try {
                ...
                gpu::GpuMat gray, blue_eq, red_eq, green_eq, minus_green, plus_blue, minus_green_eq, plus_blue_eq, minus_red_eq;

                vector<gpu::GpuMat> bgr_split;

                gpu::split(imageFromCam, bgr_split);

                gpu::equalizeHist(bgr_split[0], blue_eq);
                gpu::equalizeHist(bgr_split[1], green_eq);
                gpu::equalizeHist(bgr_split[2], red_eq);

                gpu::subtract(bgr_split[0], bgr_split[1], minus_green);
                gpu::add(minus_green, bgr_split[0], plus_blue);

                gpu::subtract(blue_eq, green_eq, minus_green_eq);
                gpu::add(minus_green_eq, blue_eq, plus_blue_eq);
                gpu::subtract(plus_blue_eq, red_eq, minus_red_eq);

                if (DEBUG_LANES) {
                    imshow("Original", (Mat)imageFromCam);
                    imshow("Blue Equalized", (Mat) blue_eq);
                    imshow("Green Equalized", (Mat)green_eq);
                    imshow("Red Equalized", (Mat)red_eq);
                    imshow("Johnny 5", (Mat)plus_blue);
                    imshow("Maybe Awesome", (Mat)minus_red_eq);
                    waitKey(30);
                }
