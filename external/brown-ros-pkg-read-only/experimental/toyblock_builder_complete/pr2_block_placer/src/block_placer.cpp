...
void BlockPlacer::executeASCB(const pr2_block_builder_msgs::PlaceCommandGoalConstPtr &goal)
{
  ...
  moveArmToPrePlacePose(goal->x,goal->y,goal->z);
  moveArmToPlacePose(goal->x,goal->y,goal->z);
...
void BlockPlacer::moveArmToCenter(int x,int y,int z)
{
  ...
  float pose[7] = {0.5,0,-0.1,0,0.707,0,0.707};
  pose[0] = pose[0] + offset * x - 0.04;
  pose[1] = pose[1] + offset * y - 0.04;
...
void BlockPlacer::moveArmToCenter2(int x,int y,int z)
{
  const float offset = 0.044;
  float pose[7] = {0.5,0,-0.1,0,0.707,0,0.707};
  pose[0] = pose[0] + offset * x;
  pose[1] = pose[1] + offset * y;
  pose[2] = pose[2] + offset * z + 0.02;
...
void BlockPlacer::moveArmToPrePlacePose(int x,int y,int z)
{
  const float offset = 0.044;
  float pose[7] = {0.5,0,-0.1,0,0.707,0,0.707};
  pose[0] = pose[0] + offset * x - 0.04;
...
void BlockPlacer::moveArmToPlacePose(int x,int y,int z)
{
  const float offset = 0.044;
  float pose[7] = {0.5,0,-0.105,0,0.707,0,0.707};
  pose[0] = pose[0] + offset * x;
  pose[1] = pose[1] + offset * y;
  pose[2] = pose[2] + (offset) * z;
