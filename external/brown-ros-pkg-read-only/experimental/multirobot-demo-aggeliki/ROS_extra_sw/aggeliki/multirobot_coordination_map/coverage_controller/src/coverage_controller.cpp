...
void nextMoveController()
{
	...
	cout << "available_next_hops service exists = " << ros::service::exists("available_next_hops", 1) << endl;
...
/*     if(!client.call(srv))
	...
	ros::service::waitForService("/available_next_hops");
	client.call(srv);
	...
	double dmax = 8; //optimal distance between 2 robots (in m) -> AIlab corridor: 8, big corridor: 13
	...
	ros::service::waitForService("/set_waypoints");
	client2.call(srv2);
	cout << "after set_waypoints" << endl;
