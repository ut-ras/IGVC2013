...
//    workerFacadeMessage({'status':'message', 'content':'Downloading ' + url});  
    ...
    if (looksLikeBinary(reader)) {
      return this.ParseSTLBinary(reader);
    } else {
      return this.ParseSTLString(file);
    }
  };
...
//    workerFacadeMessage({'status':'message', 'content':'Downloading ' + url});  
    var file = this.load_binary_resource(url);
    return this.ParseOBJString(file);
  };
...
//    workerFacadeMessage({'status':'message', 'content':'Downloading ' + url});  
    ...
    if (file.match(/format ascii/i)) {
      return this.ParsePLYString(file);
    } else {
      return this.ParsePLYBinary(file);
    }
  };
