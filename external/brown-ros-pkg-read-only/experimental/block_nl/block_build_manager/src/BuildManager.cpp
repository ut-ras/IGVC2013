    ...
    command_server(nh,COMMAND_NAME,boost::bind(&BuildManager::actionCallBack,this,_1,&(this->command_server)),false)
{
  ...
  nh.param<std::string>("object_seg_srv",service_name, "/object_filtering");
...
void BuildManager::initialize()
{
  ...
  ROS_INFO("Move Head");
  // head control
  robot.head.lookat("torso_lift_link", tf::Vector3(0.5,0.0,0.0));
...
void BuildManager::segmentation()
{
  ...
  if(!seg_srv.call(empty_call))
  {
    ROS_ERROR("Call to object_filtering failed");
  }

  ROS_INFO("Object filtering called");
}
