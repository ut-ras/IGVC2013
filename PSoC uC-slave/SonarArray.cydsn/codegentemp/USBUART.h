...
#define USBUART_MAX_REPORTID_NUMBER   0
...
#define USBUART_EP2_ISR_REMOVE                 (1u)
...
typedef struct _USBUART_EpCtlBlock
{
    uint8  attrib;
    uint8  apiEpState;
    uint8  hwEpState;
...
typedef struct _USBUART_EpSettingsBlock
{
    uint8  interface;
    uint8  altSetting;
    uint8  addr;
...
typedef struct _USBUART_XferStatusBlock
{
    uint8  status;
    uint16 length;
} T_USBUART_XFER_STATUS_BLOCK;

typedef struct _USBUART_Td
{
    uint16  count;
...
typedef struct _USBUART_BackupStruct
{
    uint8 enableState;
    uint8 mode;
...
#define USBUART_EnableInt(a, b)            CyIntEnable(a)
...
#define USBUART_DIE_ID             CYDEV_FLSHID_CUST_TABLES_BASE
    ...
    #define USBUART_PM_AVAIL_CR_PTR    (  (reg8 *) CYREG_PM_AVAIL_CR6)
    #define USBUART_PM_AVAIL_CR_REG    (* (reg8 *) CYREG_PM_AVAIL_CR6)
...
#if(CY_PSOC3_ES2 || CY_PSOC5_ES1)
    #define USBUART_PM_AVAIL_CR    USBUART_PM_AVAIL_CR_PTR
#endif /* End CY_PSOC3_ES2 || CY_PSOC5_ES1 */
...
#define USBUART_CR1_BUS_ACTIVITY           (0x01u << USBUART_CR1_BUS_ACTIVITY_SHIFT)
...
#define USBUART_CR0_ENABLE                 (0x80u)
    ...
    #define USBUART_BUS_RST_COUNT              (0x03u)
...
#define USBUART_ARB_INT_MASK              ((USBUART_DMA1_REMOVE ^ 1) | \
                                                    (USBUART_DMA2_REMOVE ^ 1) << 1 | \
                                                    (USBUART_DMA3_REMOVE ^ 1) << 2 | \
                                                    (USBUART_DMA4_REMOVE ^ 1) << 3 | \
                                                    (USBUART_DMA5_REMOVE ^ 1) << 4 | \
                                                    (USBUART_DMA6_REMOVE ^ 1) << 5 | \
                                                    (USBUART_DMA7_REMOVE ^ 1) << 6 | \
                                                    (USBUART_DMA8_REMOVE ^ 1) << 7)
