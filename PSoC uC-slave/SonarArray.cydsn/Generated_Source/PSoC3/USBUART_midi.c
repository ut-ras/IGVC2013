...
const uint8 CYCODE USBUART_MIDI_SIZE[] = {
...
volatile uint8 USBUART_midiInBuffer[USBUART_MIDI_IN_BUFF_SIZE];   /* Input endpoint buffer */
volatile uint8 USBUART_midiOutBuffer[USBUART_MIDI_OUT_BUFF_SIZE]; /* Output endpoint buffer */
...
#if (USBUART_MIDI_EXT_MODE >= USBUART_ONE_EXT_INTRF)
    volatile USBUART_MIDI_RX_STATUS USBUART_MIDI1_Event;          /* MIDI RX status structure */
    volatile uint8 USBUART_MIDI1_TxRunStat;                                /* MIDI Output running status */
    volatile uint8 USBUART_MIDI1_InqFlags;                                 /* Device inquiry flag */    
    
#if (USBUART_MIDI_EXT_MODE >= USBUART_TWO_EXT_INTRF)
    volatile USBUART_MIDI_RX_STATUS USBUART_MIDI2_Event;          /* MIDI RX status structure */
    volatile uint8 USBUART_MIDI2_TxRunStat;                                /* MIDI Output running status */
...
void USBUART_PrepareInBuffer(uint8 ic, uint8 *srcBuff, uint8 eventLen, uint8 cable)
                                                                 ;
#if (USBUART_MIDI_EXT_MODE >= USBUART_ONE_EXT_INTRF)
    void USBUART_MIDI_Init(void) ;

    uint8 USBUART_MIDI1_GetEvent(void) ;
    void USBUART_MIDI1_ProcessUsbOut(uint8 *epBuf) ;
    
    #if (USBUART_MIDI_EXT_MODE >= USBUART_TWO_EXT_INTRF)
        uint8 USBUART_MIDI2_GetEvent(void) ;
        void USBUART_MIDI2_ProcessUsbOut(uint8 *epBuf) 
                                                    ;
    #endif /* End USBUART_MIDI_EXT_MODE >= USBUART_TWO_EXT_INTRF */
#endif /* End USBUART_MIDI_EXT_MODE >= USBUART_ONE_EXT_INTRF */
...
void USBUART_MIDI_EP_Init(void) 
{
    USBUART_midiInPointer = 0u;
        ...
        USBUART_LoadInEP(USBUART_midi_in_ep, (uint8 *)USBUART_midiInBuffer,
                                                                                USBUART_MIDI_IN_BUFF_SIZE);
        /* Init DMA configurations for OUT EP*/
        USBUART_ReadOutEP(USBUART_midi_out_ep, (uint8 *)USBUART_midiOutBuffer, 
    ...
    if (USBUART_GetEPState(USBUART_midi_out_ep) == USBUART_OUT_BUFFER_FULL)
    {
        outLength = USBUART_GetEPCount(USBUART_midi_out_ep);
            ...
            outLength = USBUART_ReadOutEP(USBUART_midi_out_ep, 
                                                   (uint8 *)USBUART_midiOutBuffer, outLength);
            #if(USBUART_EP_MM == USBUART__EP_DMAMANUAL)
                do  /* wait for DMA transfer complete */
                {
                    CyDmaChStatus(USBUART_DmaChan[USBUART_midi_out_ep], NULL, &dmaState);
        ...
        if (outLength >= USBUART_EVENT_LENGTH)
        {
            ...
            while (outPointer < outLength)
            {
                ...
                /* Route USB MIDI to the External connection */
                    ...
                    {
                        USBUART_MIDI1_ProcessUsbOut((uint8 *)&USBUART_midiOutBuffer[outPointer]);
                    }
                    else if ((USBUART_midiOutBuffer[outPointer] & USBUART_CABLE_MASK) == 
                             USBUART_MIDI_CABLE_01)
                    {
                        #if (USBUART_MIDI_EXT_MODE >= USBUART_TWO_EXT_INTRF)
                            USBUART_MIDI2_ProcessUsbOut((uint8 *)&USBUART_midiOutBuffer[outPointer]);
                        #endif /* End USBUART_MIDI_EXT_MODE >= USBUART_TWO_EXT_INTRF */
                    }
                    else
                    {
                ...
                USBUART_callbackLocalMidiEvent(
                    ...
                    (uint8 *)&USBUART_midiOutBuffer[outPointer + USBUART_EVENT_BYTE1]);
...
void USBUART_MIDI_IN_EP_Service(void) 
{
    ...
    if ((USBUART_midiInPointer != 0u) && 
        (USBUART_GetEPState(USBUART_midi_in_ep) == USBUART_EVENT_PENDING))
    {
        #if(USBUART_EP_MM != USBUART__EP_DMAAUTO)
            USBUART_LoadInEP(USBUART_midi_in_ep, (uint8 *)USBUART_midiInBuffer,
            ...
            /* rearm IN EP */
            USBUART_LoadInEP(USBUART_midi_in_ep, NULL, USBUART_midiInPointer);
        #endif /* End USBUART_EP_MM != USBUART__EP_DMAAUTO*/
    ...
    */
        ...
        do 
        {
            if (USBUART_midiInPointer <= (USBUART_MIDI_IN_BUFF_SIZE - USBUART_EVENT_LENGTH))
            {
                ...
                if ((m1 = USBUART_MIDI1_GetEvent()) != 0u)
                {
            ...
            if (USBUART_midiInPointer <= (USBUART_MIDI_IN_BUFF_SIZE - USBUART_EVENT_LENGTH))
            {
                ...
                if ((m2 = USBUART_MIDI2_GetEvent()) != 0u)
                {
             ...
             (USBUART_midiInPointer <= (USBUART_MIDI_IN_BUFF_SIZE - USBUART_EVENT_LENGTH)));
    #endif /* End USBUART_MIDI_EXT_MODE >= USBUART_ONE_EXT_INTRF */
...
*
*******************************************************************************/
uint8 USBUART_PutUsbMidiIn(uint8 ic, uint8* midiMsg, uint8 cable)
                                                            
{
                ...
                (USBUART_EP[USBUART_midi_in_ep].bufferSize - USBUART_EVENT_LENGTH))
    {
        ...
        {   /* only SysEx message is greater than 4 bytes */
            do
            {
                USBUART_PrepareInBuffer(USBUART_MIDI_SYSEX, midiMsg, 
                                                 USBUART_EVENT_BYTE3, cable);
                ic -= USBUART_EVENT_BYTE3;
                midiMsg += USBUART_EVENT_BYTE3;
                if (USBUART_midiInPointer > 
                    (USBUART_EP[USBUART_midi_in_ep].bufferSize - USBUART_EVENT_LENGTH))
                {
                    USBUART_MIDI_IN_EP_Service();
    ...
    else if((ic < USBUART_EVENT_LENGTH) || (ic == USBUART_MIDI_SYSEX))
    {
        ...
        else if (srcBuffZero < USBUART_MIDI_SYSEX)
        {
            USBUART_midiInBuffer[USBUART_midiInPointer] =
                                                    ((srcBuffZero >> 4u) & USBUART_CIN_MASK) | cable;
            USBUART_midiInPointer++;
        }
                                                                ...
                                                                
    {
        ...
        */
        if ((mData & USBUART_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            ...
            {
                if ((mData & USBUART_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(singl byte) */
                {
                    midiReturn = mData;
                }
