...
/*******************************************************************************
* Function Name: Encoder_Clock_2_Start
********************************************************************************
* Summary:
...
void Encoder_Clock_2_Start(void) 
{
    ...
    Encoder_Clock_2_CLKEN |= Encoder_Clock_2_CLKEN_MASK;
}


/*******************************************************************************
* Function Name: Encoder_Clock_2_Stop
********************************************************************************
* Summary:
...
void Encoder_Clock_2_Stop(void) 
{
	...
	CYDEV_CHIP_REVISION_USED == CYDEV_CHIP_REVISION_5A_ES1))
/*******************************************************************************
* Function Name: Encoder_Clock_2_StopBlock
********************************************************************************
* Summary:
...
{
    if (Encoder_Clock_2_CLKEN & Encoder_Clock_2_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = Encoder_Clock_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Encoder_Clock_2_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Encoder_Clock_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Encoder_Clock_2_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: Encoder_Clock_2_StandbyPower
********************************************************************************
* Summary:
...
void Encoder_Clock_2_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        Encoder_Clock_2_CLKSTBY &= ~Encoder_Clock_2_CLKSTBY_MASK;
    }
    else
    {
        Encoder_Clock_2_CLKSTBY |= Encoder_Clock_2_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: Encoder_Clock_2_SetDividerRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Encoder_Clock_2_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    uint8 enabled;

    uint8 currSrc = Encoder_Clock_2_GetSourceRegister();
    uint16 oldDivider = Encoder_Clock_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Encoder_Clock_2_CLKEN & Encoder_Clock_2_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(Encoder_Clock_2_DIV_PTR, clkDivider);
                Encoder_Clock_2_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
...
/*******************************************************************************
* Function Name: Encoder_Clock_2_GetDividerRegister
********************************************************************************
* Summary:
...
{
    return CY_GET_REG16(Encoder_Clock_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: Encoder_Clock_2_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Encoder_Clock_2_SetModeRegister(uint8 clkMode) 
{
    Encoder_Clock_2_MOD_SRC |= clkMode & Encoder_Clock_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: Encoder_Clock_2_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Encoder_Clock_2_ClearModeRegister(uint8 clkMode) 
{
    Encoder_Clock_2_MOD_SRC &= ~clkMode | ~Encoder_Clock_2_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: Encoder_Clock_2_GetModeRegister
********************************************************************************
* Summary:
...
{
    return Encoder_Clock_2_MOD_SRC & Encoder_Clock_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: Encoder_Clock_2_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = Encoder_Clock_2_GetDividerRegister();
    uint8 oldSrc = Encoder_Clock_2_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        Encoder_Clock_2_MOD_SRC |= CYCLK_SSS;
        Encoder_Clock_2_MOD_SRC =
            (Encoder_Clock_2_MOD_SRC & ~Encoder_Clock_2_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Encoder_Clock_2_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return Encoder_Clock_2_MOD_SRC & Encoder_Clock_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Encoder_Clock_2_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    Encoder_Clock_2_PHASE = clkPhase & Encoder_Clock_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Encoder_Clock_2_GetPhase
********************************************************************************
* Summary:
...
{
    return Encoder_Clock_2_PHASE & Encoder_Clock_2_PHASE_MASK;
}
